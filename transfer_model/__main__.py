...
from .utils import read_deformation_transfer, np_mesh_to_o3d, process_rotation_matrix, pkl_to_amass
...
def main() -> None:
    ...
    if osp.exists(mask_ids_fname):
        ...
        logger.error(f'Mask ids fname not found: {mask_ids_fname}')
    ...
    for ii, batch in enumerate(tqdm(dataloader)):
        ...
        var_dict = run_fitting(exp_cfg, batch, body_model, def_matrix, mask_ids, init_param=True, smplh_path="/scratch/stu5/Data/BMLhandball/smplh/S02_Novice/Trial_upper_left_005_poses.npz")
        paths = batch['paths']

        for ii, path in enumerate(paths):
            _, fname = osp.split(path)

            output_path = osp.join(output_folder, f'{osp.splitext(fname)[0]}.npz')
            logger.info(f'Saving to: {output_path}')

            pkl_to_amass(var_dict, output_path)
