...
import sys
sys.path.append("/scratch/stu5/research_jul_28/Project/Motion/smplx")
import os
...
def main() -> None:
    ...
    body_model = smplx.create(model_path, **exp_cfg.body_model)
    logger.info(body_model)
    body_model = body_model.to(device=device)

    deformation_transfer_path = exp_cfg.get('deformation_transfer_path', '')
    # build the deformation matrix (the matrix build the function between smplx and smpl)
    ...
    for ii, batch in enumerate(tqdm(dataloader)):
        ...
        for ii, path in enumerate(paths):
            ...
            # write the pkl file 
            # pkl file contains all the infomation that we need
